# Plan - Integrate Mammalian Phenotype Ontology into terminology and translation support

This plan introduces Mammalian Phenotype Ontology into the project where it can improve terminology alignment, multilingual review, or validation.

## Implementation Result
Phase 0 governance is implemented as metadata-only and remains blocked before payload ingestion. Mammalian Phenotype Ontology may only be used for source-authority, version-pinning, and cross-species phenotype governance scaffolding until terms review confirms the release channel, version, and redistribution status.
...
- [x] **Task 1:** Define source retrieval path without committing restricted payloads.
  - Use the pinned MGI release metadata and local-only cache; do not fetch or commit term payloads before release-specific licence review.
...
- [x] **Task 1:** Identify Mammalian Phenotype use for phenotype-domain validation, identifier conflict detection, and cross-species context; terms remain blocked.
...
- [x] **Task 1:** Validate governance schema, provenance, and licence metadata; source terms remain blocked.
...
Until source terms and payload handling are cleared, commit metadata-only scaffolding only: registry records, source-access records, provenance shells, local-only manifest references, schemas, validation summaries, agent-panel assessment packs, and accountable-maintainer decision records.
