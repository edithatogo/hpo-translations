...
- [~] **Task 4:** Close source, license, ethics, privacy, reviewer-consent, language-working-group, and community model-use gates. The fail-closed G1/G2 docket defines ten evidence packets, owner roles, and step-down actions; unsent permission, consultation, reviewer-interest, and ethics/privacy drafts plus a blank decision-receipt template are prepared. Every decision remains pending and no external contact is authorized. Step down A → B → C → D when a required gate cannot close. (`5e8ff7f`, `8070f4d`)
