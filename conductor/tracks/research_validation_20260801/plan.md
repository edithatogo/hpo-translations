...
- [~] **Task 4:** Close source, license, ethics, privacy, reviewer-consent, language-working-group, and community model-use gates. The fail-closed G1/G2 decision docket now defines ten evidence packets, owner roles, and step-down actions; every decision remains pending and no external contact is authorized. Step down A → B → C → D when a required gate cannot close. (`phase_4_gate_docket.json`)
