...
- [~] **Task 2:** Pin the HPO release and every source release selected for the pilot; complete source access, license, credential, source-authority, language, and version checks. HPO `v2026-06-23`, the four official mapping families, and nine supplementary candidates are reviewed. Payload-free archive receipts pin Disease Ontology `v2026-07-31`, MP `v2026-07-22`, PATO `v2025-05-14`, and uPheno `v2025-10-12` to checksums at one owner-only archive revision. A canonical readiness matrix now reconciles these three evidence layers and prevents PATO, MP, or uPheno archive overlap from creating false source independence. The evidence establishes metadata, version, and integrity readiness only: the final pilot source set is not selected, no source atoms are ready, and no payload source is authorized. DeCS version, remaining provider permissions, credential custody, and accountable human licence decisions remain open. (`3ff429e`, review fix `e410f47`, `f3a9584`)
...
- [~] **Task 3:** Populate derivation paths, shared-lineage clusters, and independent-evidence groups at the originating source-atom level. Metadata-level lineage and four immutable private-archive version/checksum receipts are complete; the receipts contain no source atoms and confer no evidence independence. Source-atom ingestion remains blocked on payload, licence, attribution, and pilot-scope approval. (`3ff429e`)
...
- [x] **Task 3:** Prepare the metadata-only language, reviewer-role, source-pathway, and contingency matrix. Spanish and Japanese remain planning preferences, the community-governed slot remains unassigned, and `tw` is excluded. Permitted selection, named roster, payload-authorized source set, and stratified concept freeze remain blocked on G1/G2 approvals. (`da81298`)
- [~] **Task 4:** Close source, licence, ethics, privacy, reviewer-consent, language-working-group, and community model-use gates. Wave 1 is complete with four conditional, version-pinned internal planning scopes. Wave 2 official routes and bounded request drafts are prepared for separate Spanish, Japanese, and ethics/privacy decisions, but all three remain pending and unsent. Dispatch requires the accountable sponsor route, an authorized off-repository sender reference, and a language contact mode. PRO-CTCAE and DeCS remain externally blocked on sender identity; payload retrieval, reviewer contact, empirical work, and promotion remain blocked. (`5e8ff7f`, `8070f4d`, `176e7e2`, `8d0ca96`, blocker plan `b4f31f7`, Wave 1 `7881787`, Wave 2 `9953018`, routing receipt `68be472`)
- [~] **Task 5:** Prepare, then freeze and checksum, candidate conditions, prompts, model endpoints, source versions, randomization, exclusions, instrument, progression criteria, and analysis code. The deterministic G3 readiness contract is prepared but explicitly not frozen; its approval counts, authority state, and G1/G2 decisions are reconciled against canonical inputs and fail closed on drift. A 12-component inventory records each intended freeze artifact, planning source, readiness state, and blocker with zero hashes or ready components. An unexecuted receipt template mirrors every canonical freeze field while retaining null values and no authority. G1/G2 evidence and explicit maintainer approval remain prerequisites to an immutable executed receipt and any external preregistration. (`12c6ad6`, reconciliation `639b2ce`, inventory `9bd2903`, receipt template `dd97fbb`)
...
- [x] **Task:** Supersede the planned human reviewer and adjudicator pathway with the canonical isolated specialist-agent panel in `research_validation/agent_review_panel.json`. Preserve source rights, community-use constraints, prospective freeze, and maintainer release authority as non-review gates; prohibit human-validation claims. (current continuation)

### Review outcome

The governance and validation continuation is locally admitted, but the track is not complete or archive-eligible. G1 source/payload decisions, applicable language/community-use constraints, the executed G3 freeze, Stage 1 agent-panel evaluation, the full pilot, confirmatory studies, downstream delivery, explicit publication approval, current-head CI, and merge verification remain outstanding. Human review is not part of the revised design, and the study must not make human-, community-, or clinical-validation claims. The already-executed credentialed archive run also requires an authorized audit and, if necessary, remediation because its pre-fix receipts could contain a complete authenticated metadata response. Archival must not convert these empirical and accountable gates into completed work.
