...
- [x] **Task 2:** Pin the HPO release and every source release selected for the pilot; complete source access, licence, source-authority, language, and version checks. The exact minimum payload set is the content-addressed Spanish and Japanese Babelon snapshots, and the repository maintainer has approved their bounded study use under the official HPO licence conditions in `spanish_japanese_use_approval.json`. DeCS and every external or restricted payload remain omitted. Future external-source lineage and current-release alignment claims remain separately blocked. (`3ff429e`, review fix `e410f47`, `f3a9584`, current continuation)
...
- [x] **Task 3:** Prepare the metadata-only language, agent-role, source-pathway, and contingency matrix. Spanish and Japanese are selected and subsequently approved for the exact frozen Option B scope; the third community-governed slot is removed. The unresolved `tw` profile is canonically excluded from every study stage, source match, fallback, analysis, and claim without renaming or inferring its identity. (`da81298`, current continuation)
- [x] **Task 4:** Close source, licence, ethics, privacy and agent-execution gates. The repository maintainer approved bounded use of the exact Spanish/Japanese snapshots under the official HPO licence conditions. The study sponsor determined that no formal ethics or privacy application is required for the frozen no-person, no-personal-data scope. The post-freeze receipts close the Stage 1 execution gates without changing G3, claiming community validation, authorizing promotion, or permitting upstream writes. (`5e8ff7f`, `8070f4d`, `176e7e2`, `8d0ca96`, blocker plan `b4f31f7`, Wave 1 `7881787`, Wave 2 `9953018`, current continuation)
- [x] **Task 5:** Freeze and checksum candidate conditions, prompts, model endpoints, source versions, sampling, randomization, aggregation, exclusions, the specialist-agent instrument, progression criteria, approval/privacy controls, and analysis code. Freeze `g3-option-b-es-ja-20260812-v1` binds 13 immutable components for the 50-concept/60-unit Spanish–Japanese Option B design. The later language-use and ethics/privacy receipts leave those components unchanged. Stage 1 may now execute within that exact scope; external preregistration remains separately prohibited. Any component drift requires a new freeze identifier.
- [~] **Task 6:** Run five context-isolated specialist agent assessments and independent agent adjudication for the first 12 authorized concept-language units, balanced across approved languages and including at least two common anchors where the design is multilingual, then reproduce the deterministic aggregation and apply the frozen go, revise, or stop rule without testing candidate-method superiority. The deterministic preflight now verifies all 13 G3 hashes, 12 units, 48 condition rows, five specialist roles, isolated adjudicator, auditor, and both post-freeze receipts; it records that execution has not started. Do not recruit people or describe the result as human, community, or clinical validation.
...
- [x] **Task:** Add a continuous prospective improvement register covering every source assignment and A1-A10 analysis, retain negative and non-estimable evidence, add the non-generative evidence-only model baseline, and require an amendment or new freeze before empirical inputs change. (current continuation)
- [~] **Task:** Archive the full datasets behind every mapping-catalog artifact using checksum-pinned public, private-Hugging-Face, local-only, or metadata-only routing. Three permissive datasets are archived in ignored local staging; remote Hugging Face upload is blocked pending token rotation, and restricted/unresolved payloads remain unretrieved until their exact storage and redistribution terms permit the selected route. (current continuation)
...
- [x] **Task:** Define and validate BabelNet as a conditional, lineage-aware linguistic tie-breaker for pivot disagreement, lexical gaps, descriptive components and unresolved Portuguese locale. Prohibit full-scan default use, first-result selection, generic-`pt` regional inference, API execution, payload use and empirical claims until exact licence, credentials and a new freeze close. (current continuation)
