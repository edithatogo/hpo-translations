...
- `biomedical_source_preparation_matrix.json` decomposes the requested MAxO/HPO associations, measurement and provenance ontologies, GA4GH resources, multilingual/context candidates, and downstream controls into 22 individually gated components and five prospective analysis groups. It authorizes no retrieval or empirical use. Run `pixi run validate-biomedical-source-preparation` after editing it.
- `ga4gh_relevance_catalog.json` applies a relevance-first policy to 14 GA4GH standards and datasets. Current, draft, developmental and study-group products are retained whenever an explicit, documented, proposed or testable path connects them to HPO phenotypes, provenance, genomic interpretation, cohort discovery or data-use governance. Maturity never substitutes for mapping evidence or execution authority. Run `pixi run validate-ga4gh-relevance` after editing it.
- `snomed_icd_crosslingual_bridge_catalog.json` governs UMLS, authority-published SNOMED map products, OMOP/Athena, Orphadata and MedDRA as cross-lingual SNOMED–ICD bridge resources. It distinguishes direct maps from metathesaurus, standardization and terminology-pivot paths, preserves edition and language boundaries, and authorizes no authenticated access or payload. Run `pixi run validate-snomed-icd-bridges` after editing it.
- `babelnet_linguistic_anchor_plan.json` governs BabelNet as a conditional linguistic tie-breaker and gap resolver. It preserves originating-resource lineage, treats Health domains as one feature rather than a sufficient filter, separates generic Portuguese from `pt-BR` and `pt-PT`, and requires a new freeze before API use or empirical analysis. Run `pixi run validate-babelnet-linguistic-anchor` after editing it.
